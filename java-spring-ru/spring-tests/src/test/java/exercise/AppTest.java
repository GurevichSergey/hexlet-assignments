    ...
    void testUpdatePerson() throws Exception {
        MockHttpServletResponse responseUpdate = mockMvc
                .perform(
                        patch("/people/5")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("{\"firstName\": \"Sergey\", \"lastName\": \"Golovach\"}")
                )
                .andReturn()
                .getResponse();
        ...
        assertThat(response.getContentType()).isEqualTo(MediaType.APPLICATION_JSON.toString());
        assertThat(response.getContentAsString()).contains("Sergey", "Golovach");
        assertThat(response.getContentAsString()).doesNotContain("Gurevich");
    }
    @Test
